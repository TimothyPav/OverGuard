...
std::unordered_map<std::string, std::variant<int, double>>
get_column(const std::string hero_name, const std::string column) {
  sqlite3 *hero_db;
  ...
  const std::string sql_select =
      "SELECT \"" + column + "\",Name FROM " + hero_name;
  // const std::string sql_select = "SELECT Solo Kills per 10 mins, Name FROM
  // widowmaker";
  sqlite3_stmt *stmt;
  return_code =
      sqlite3_prepare_v2(hero_db, sql_select.c_str(), -1, &stmt, NULL);
...
int load_players_into_playersDatabase(std::string blizzard_ID) {
  ...
  const char *sql_insert =
      "INSERT INTO players (Name,Blizzard_ID) VALUES (?,?);";
...
std::string
select_name_from_blizzard_ID_from_database(std::string blizzard_ID) {
  sqlite3 *players_db;
  char *error_message = 0;
  sqlite3_stmt *stmt;
...
int insert_stats_into_asheDatabase(std::string blizzard_ID, Heroes &ashe) {
  ...
  int delete_return_code =
      sqlite3_prepare_v2(ashe_db, sql_delete.c_str(), -1, &stmt, nullptr);
  ...
  // sql command
  const char *sql_insert =
      "INSERT INTO ashe (\"Blizzard_ID\", \"Name\", \"Deaths per 10 mins\", "
      "\"Final Blows per 10 mins\", \"Solo Kills per 10 mins\", \"Hero Damage "
      "Done per 10 mins\", \"Scoped Accuracy\", \"Scoped Crit Accuracy\", "
      "\"Scoped Crit Hits per 10 mins\", \"Scoped Crit Kills per 10 mins\") "
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
  ...
  sqlite3_bind_text(stmt, 1, blizzard_ID.c_str(), -1, SQLITE_TRANSIENT);
  sqlite3_bind_text(
      stmt, 2, select_name_from_blizzard_ID_from_database(blizzard_ID).c_str(),
      -1, SQLITE_TRANSIENT);
...
int insert_stats_into_cassidyDatabase(std::string blizzard_ID,
                                      Heroes &cassidy) {
  sqlite3 *cassidy_db;
  char *error_message = 0;
  sqlite3_stmt *stmt;
  ...
  int delete_return_code =
      sqlite3_prepare_v2(cassidy_db, sql_delete.c_str(), -1, &stmt, nullptr);
  ...
  // sql command
  const char *sql_insert =
      "INSERT INTO cassidy (\"Blizzard_ID\", \"Name\", \"Deaths per 10 mins\", "
      "\"Final Blows per 10 mins\", \"Solo Kills per 10 mins\", \"Hero Damage "
      "Done per 10 mins\", \"Critical Hit Accuracy\", \"Critical Hits per 10 "
      "mins\", \"Critical Hit kills per 10 mins\") VALUES (?,?,?,?,?,?,?,?,?);";
...
int insert_stats_into_hanzoDatabase(std::string blizzard_ID, Heroes &hanzo) {
  ...
  int delete_return_code =
      sqlite3_prepare_v2(hanzo_db, sql_delete.c_str(), -1, &stmt, nullptr);
  ...
  // sql command
  const char *sql_insert =
      "INSERT INTO hanzo (\"Blizzard_ID\", \"Name\", \"Deaths per 10 mins\", "
      "\"Final Blows per 10 mins\", \"Solo Kills per 10 mins\", \"Hero Damage "
      "Done per 10 mins\", \"Critical Hit Accuracy\", \"Critical Hits per 10 "
      "mins\", \"Critical Hit kills per 10 mins\") VALUES (?,?,?,?,?,?,?,?,?);";
...
int insert_stats_into_sojournDatabase(std::string blizzard_ID,
                                      Heroes &sojourn) {
  sqlite3 *sojourn_db;
  char *error_message = 0;
  sqlite3_stmt *stmt;
  ...
  int delete_return_code =
      sqlite3_prepare_v2(sojourn_db, sql_delete.c_str(), -1, &stmt, nullptr);
  ...
  // sql command
  const char *sql_insert =
      "INSERT INTO sojourn (\"Blizzard_ID\", \"Name\", \"Deaths per 10 mins\", "
      "\"Final Blows per 10 mins\", \"Solo Kills per 10 mins\", \"Hero Damage "
      "Done per 10 mins\", \"Charged Shot Accuracy\", \"Charged Shot Crit "
      "Accuracy\", \"Charged Shot Kills per 10 mins\") VALUES "
      "(?,?,?,?,?,?,?,?,?);";
...
int insert_stats_into_soldier76Database(std::string blizzard_ID,
                                        Heroes &soldier76) {
  ...
  int delete_return_code =
      sqlite3_prepare_v2(soldier76_db, sql_delete.c_str(), -1, &stmt, nullptr);
  ...
  // sql command
  const char *sql_insert =
      "INSERT INTO soldier76 (\"Blizzard_ID\", \"Name\", \"Deaths per 10 "
      "mins\", \"Final Blows per 10 mins\", \"Solo Kills per 10 mins\", \"Hero "
      "Damage Done per 10 mins\", \"Critical Hit Accuracy\", \"Critical Hits "
      "per 10 mins\") VALUES (?,?,?,?,?,?,?,?);";
  return_code =
      sqlite3_prepare_v2(soldier76_db, sql_insert, -1, &stmt, nullptr);
...
int insert_stats_into_tracerDatabase(std::string blizzard_ID, Heroes &tracer) {
  ...
  int delete_return_code =
      sqlite3_prepare_v2(tracer_db, sql_delete.c_str(), -1, &stmt, nullptr);
  ...
  // sql command
  const char *sql_insert =
      "INSERT INTO tracer (\"Blizzard_ID\", \"Name\", \"Deaths per 10 mins\", "
      "\"Final Blows per 10 mins\", \"Solo Kills per 10 mins\", \"Hero Damage "
      "Done per 10 mins\", \"Critical Hits per 10 mins\") VALUES "
      "(?,?,?,?,?,?,?);";
...
int insert_stats_into_widowmakerDatabase(std::string blizzard_ID,
                                         Heroes &widowmaker) {
  ...
  int delete_return_code =
      sqlite3_prepare_v2(widowmaker_db, sql_delete.c_str(), -1, &stmt, nullptr);
  ...
  // sql command
  const char *sql_insert =
      "INSERT INTO widowmaker (\"Blizzard_ID\", \"Name\", \"Deaths per 10 "
      "mins\", \"Final Blows per 10 mins\", \"Solo Kills per 10 mins\", \"Hero "
      "Damage Done per 10 mins\", \"Scoped Accuracy\", \"Scoped Crit "
      "Accuracy\", \"Scoped Crit Hits per 10 mins\", \"Scoped Crit Kills per "
      "10 mins\") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";

  return_code =
      sqlite3_prepare_v2(widowmaker_db, sql_insert, -1, &stmt, nullptr);
